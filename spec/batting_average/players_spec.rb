...
RSpec.describe BattingAverage, '#players' do
  ...
  context 'with filter' do
    ...
    context 'when year filter is given as number' do
      ...
      it 'extract players data' do
        expect(batting_average.players.map { |data| data[:player_id] }).to eq %w[allisar01 allisar01]
      end
    end
    ...
    context 'when team_name is given' do
      ...
      it 'extract players data' do
        expect(batting_average.players.map { |data| data[:player_id] }).to eq ['abercda01']
      end
    end
    ...
    context 'when year and team_name is given' do
      ...
      it 'extract players data' do
        expect(batting_average.players.map { |data| data[:player_id] }).to eq ['addybo01']
      end
    end
  end
end
